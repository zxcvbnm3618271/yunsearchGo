...
public class ESSyner {
    ...
    //改为使用logstash-input-jdbc插件进行自动同步，放弃Schedule方法
//    @Scheduled(cron = "10/20 * * * * ?")
    public void mysql2Elastic() throws IOException {
        ...
        try {
            ...
            yunDatas = yunDataService.findByUpdateTime(yunData.getUpdateAt());
        } catch (Exception e) {
...
//        Set<YunData> uniqueDataSet = new HashSet<>();
        ...
        for (YunData data : yunDatas) {
            ...
            duplicateDataSet.add(data);
//            //获取全部同一个人分享的文件
//            List<YunData> duplicateDataList = yunDataService.findByUserId(data.getUserId());
//
//            //分类：最新的资源
